...
{
    /// <summary>Stream wrapper que no cierra el stream subyacente al hacer Dispose (NPOI cierra el stream al hacer Write).</summary>
    internal sealed class NonClosingStream : Stream
    {
        private readonly Stream _inner;
        public NonClosingStream(Stream inner) => _inner = inner ?? throw new ArgumentNullException(nameof(inner));
        ...
        protected override void Dispose(bool disposing) { /* No cerrar _inner; el caller es dueño */ }
    }
...
<DeviceInfo>
  <SimplePageHeaders>True</SimplePageHeaders>
            ...
            out string fileName)
        {
            ...
            // Preparar stream (NPOI puede cerrar el stream al hacer Write; usar wrapper para que siga usable)
            var stream = new MemoryStream();
            using (var nonClosing = new NonClosingStream(stream))
            {
                workbook.Write(nonClosing);
            }
            stream.Position = 0;
